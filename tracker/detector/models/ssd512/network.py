...
def get_detection_head(num_classes):
    classification_head_layers = [
        ...
        layers.Conv2D(4 * num_classes, 3, padding='same', use_bias=False, kernel_initializer=initializer,
                      name='classifier5'),
        layers.Conv2D(4 * num_classes, 1, use_bias=False, kernel_initializer=initializer, name='classifier6')
    ]
    ...
    localization_head_layers = [
        ...
        layers.Conv2D(4 * 4, 3, padding='same', use_bias=False, kernel_initializer=initializer, name='localizer5'),
        layers.Conv2D(4 * 4, 1, use_bias=False, kernel_initializer=initializer, name='localizer6')
    ]
...
def get_network(num_classes):
    ...
    print('Using SSD ssd512 network with {} classes including Background'.format(num_classes))
    return model
